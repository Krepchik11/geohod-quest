...
await pageA.locator('.auth-policy').click();
await pageA.locator('button.btn', { hasText: 'Зарегистрироваться' }).click();
await pageA.locator('.auth-title', { hasText: 'Вы вошли' }).waitFor({ timeout: 10000 });
...
check('registration kept the anonymous player id', sessionA?.player_id === anonId,
  `${sessionA?.player_id} vs ${anonId}`);
...
const spoof = await fetch(`${API}/api/checkout`, {
  ...
  body: JSON.stringify({ player_id: anonId, quest_id: 'mystery-fortress-v1' }),
});
...
await pageB.locator('button.btn', { hasText: 'Вход' }).first().click();
await pageB.fill('input[type=email]', EMAIL);
await pageB.fill('input[type=password]', PASSWORD);
await pageB.locator('button.btn', { hasText: /^Войти$/ }).click();
await pageB.locator('.auth-title', { hasText: 'Вы вошли' }).waitFor({ timeout: 10000 });
const sessionB = await pageB.evaluate(() => JSON.parse(localStorage.getItem('geohod-session:v1')));
check('device B adopted device A identity', sessionB?.player_id === anonId,
  `${sessionB?.player_id} vs ${anonId}`);
...
await pageC.locator('button.btn', { hasText: 'Вход' }).first().click();
await pageC.fill('input[type=email]', EMAIL);
await pageC.fill('input[type=password]', 'wrong-password-1');
await pageC.locator('button.btn', { hasText: /^Войти$/ }).click();
await pageC.locator('.auth-error', { hasText: 'Неверный email или пароль' }).waitFor({ timeout: 10000 });
check('wrong password shows the 401 error and no session', true);
