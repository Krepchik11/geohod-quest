...
pub struct InMemoryFactStore {
    ...
    /// Store-wide append order, parallel to `fact_logs` like `fact_times`
    /// (mirrors the Postgres `facts.seq` BIGSERIAL). Receive times have
    /// seconds granularity, so "newest" needs this as the tiebreaker to stay
    /// deterministic — exactly the `(recorded_at, seq)` order Postgres uses.
    fact_seqs: HashMap<String, Vec<u64>>,
    next_fact_seq: u64,
...
impl InMemoryFactStore {
    ...
    pub fn append_idempotent(
        ...
        for f in incoming {
            ...
            self.fact_times
                ...
                .or_default()
                .push(now_secs());
            self.next_fact_seq += 1;
            self.fact_seqs
                .entry(attempt_id.to_string())
                .or_default()
                .push(self.next_fact_seq);
            accepted.push(f);
        }
        Some(accepted)
    }
    ...
    pub fn quest_rating_rows(
        ...
        for meta in self.attempts.values() {
            ...
            let seqs = self.fact_seqs.get(&meta.attempt_id);
            for (i, f) in log.iter().enumerate() {
                ...
                let seq = seqs.and_then(|s| s.get(i).copied()).unwrap_or(0);
                let key = (meta.quest_id.clone(), meta.user_id.clone());
                let entry = best.entry(key).or_default();
                if is_newer(&entry.0, at, seq) {
                    entry.0 = Some((at, seq, crate::facts::parse_rating(f)));
                }
                let text = f
                    .note
                    .as_deref()
                ...
                if let Some(text) = text
                    && is_newer(&entry.1, at, seq)
                {
                    entry.1 = Some((at, seq, text));
                }
            }
        }
        best.into_iter()
            .filter_map(|((quest_id, user_id), (stars, text))| {
                ...
                let (text_at, text_seq, text) = match text {
                    Some((at, seq, t)) => (at, seq, Some(t)),
                    None => (0, 0, None),
                };
